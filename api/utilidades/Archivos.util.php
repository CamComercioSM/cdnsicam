...
class Archivos {
    ...
    public static function copiarAOtraCarpeta($src, $capetaDESTINO, $nombreDESTINO) {
        $dst = $capetaDESTINO . $nombreDESTINO;
        if (file_exists($dst)) {
            rrmdir($dst);
        }
        if (is_dir($src)) {
            ...
            self::probarCrearDirectorio($capetaDESTINO);
            return copy($src, $dst);
        }
        return null;
    }

    public static function convertirPATHlocalURLsicam($documentoURLsicam) {
        return str_replace([DIR_ARCHIVOS, DIR_API, DS, DIR_COMPONENTES], [URL_ARCHIVOS, URL_API, WS, URL_COMPONENTES], $documentoURLsicam);
    }

    public static function convertirURLsicamPATHlocal($documentoURLsicam) {
        return str_replace([URL_ARCHIVOS, WS], [DIR_ARCHIVOS, DS], $documentoURLsicam);
    }

    public static function guardarImagenBase64($imgBase64, $nombreArchivo, $carpetaArchivo) {
    ...
    public static function guardarImagenWEB($url, $guardarEn) {
        $ch = curl_init($url);
        ...
        $guardado = fwrite($fp, $raw);
        fclose($fp);
        return $guardado;
    }

    public static function generarQRparaURL($url, $guardarEn) {
//   echo $URLGeneradorQR = 'https://chart.googleapis.com/chart?chs=330x330&cht=qr&chld=L|0&chl='
//        . $url. "";
        $URLGeneradorQR = 'https://quickchart.io/qr?text='. $url . '&ecLevel=L&size=350';
        return Archivos::guardarImagenWEB($URLGeneradorQR, DIR_ARCHIVOS . $guardarEn);
    }

    public static function conteoArchivo($dirArchivo) {
    ...
    public static function nombreExtension($archivo) {
        return $ext = pathinfo($archivo['name'], PATHINFO_EXTENSION);
    }

    public static function extension($archivo) {
        return $ext = pathinfo($archivo['name'], PATHINFO_EXTENSION);
    }

    public static function extensionRUTA($archivo) {
        return $ext = pathinfo($archivo, PATHINFO_EXTENSION);
    }
    ...
    static public function leerArchivo($archivo) {
    ...
    static public function leerLineaArchivo($archivo, $linea = 0) {
        $leida = "";
        $numLinea = 0;
        if (file_exists($archivo)) {
    ...
    static public function borrarLineaArchivo($archivo, $linea = 0) {
        $leidas = array();
        $numLinea = 0;
        if (file_exists($archivo)) {
            $file = fopen($archivo, "r");
    ...
    static public function leerArchivoPlanoSeparadoPorLinea($archivo, $separador = ";") {
    ...
    static public function leerArchivoPlanoSeparadoPorLineaEncabezado($archivo, $separador = ";") {
    ...
    static public
      function escribirLog($archivo, $texto) {
        if (!file_exists(DIR_LOGS . $archivo)) {
            $file = fopen(DIR_LOGS . $archivo, "c+");
//            fwrite($file, $texto. "; " .Usuarios::mensajes_sistema() . PHP_EOL );
            fwrite($file, $texto . "; " . PHP_EOL);
            fclose($file);
        } else {
            $file = fopen(DIR_LOGS . $archivo, "r+");
            $contenido = fread($file, intval(filesize(DIR_LOGS . $archivo)));
            rewind($file);
//            fwrite($file, $texto . "; " . Usuarios::mensajes_sistema() . PHP_EOL. $contenido  );
    ...
    public static function probarCrearDirectorio($ruta, $Separador = DS) {
        $ok = false;
        ...
        foreach ($carpetas as $carpeta) {
            ...
            if (!is_dir($rActual)) {
                ...
                if (mkdir($rActual, 0777)) {
                    if (!is_file($rActual . "index.html")) {
                        $HTMLenblanco = fopen($rActual . "index.html", "w+");
                        chmod($rActual . "index.html", 0774);
                        fwrite($HTMLenblanco, '<html></html>');
                        fclose($HTMLenblanco);
                    }
                    $ok = true;
                }
